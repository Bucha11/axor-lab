...
used to hold its own copies and was documented as "the de-facto axor-core
baseline" because the kernel did not ship them — it does now, byte-identical
to what was here, so the copies are gone and these three are imported. A
schema with two definitions has none.

The other six — `attestation`, `bundle`, `condition`, `experiment`,
`publication`, `scenario` — are the Lab's own product and stay in
`contracts/`, which remains their source of truth; this module only loads
them. Resolution order for that directory: an explicit argument, the
AXOR_LAB_CONTRACTS environment variable, package data (installed wheel), then
the repo-relative default (this package sits next to `contracts/`).

All nine are returned in ONE dictionary because they reference each other
...
def load_schemas(directory: str | None = None) -> dict[str, dict[str, object]]:
    ...
    The Lab's six come from disk (explicit `directory` arg → AXOR_LAB_CONTRACTS
