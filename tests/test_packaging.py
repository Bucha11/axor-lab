...
Only the Lab's six are in either place. `trace`, `tool-manifest` and
...
class TestTheKernelSchemasAreNotKeptHere(unittest.TestCase):
    ...
    def test_a_stray_copy_could_not_shadow_the_owner(self) -> None:
        """The loader merges the kernel's over the Lab's, so a file left behind
        under one of these names loses. Nothing depends on nobody ever making
        that mistake."""
        from axor_core.contracts.schemas import load

        from lab_contracts.schemas import KERNEL_SCHEMA_NAMES as kernel
        from lab_contracts.schemas import load_schemas

        load_schemas.cache_clear()
        try:
            merged = load_schemas(str(REPO_ROOT / "contracts"))
            for name in kernel:
                self.assertEqual(merged[name], load(name), name)
        finally:
            load_schemas.cache_clear()
