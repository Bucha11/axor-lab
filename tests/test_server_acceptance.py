...
(review r15; simplified for spec v0.3).

The publish response and the download carry a portable acceptance receipt — the
server's attestation of what it verified — persisted and, when the server has a
key, Ed25519-signed. v0.3 keeps *publication + bundle hash + optional signature +
reproduction records*; the r18-r21 reacceptance / acceptance-history / quarantine
chains are deferred, so a persisted acceptance is restored verbatim (never
re-minted under a rotated key) or, if absent/damaged, freshly minted.
"""
