...
class TestCandidateRealKernel(unittest.TestCase):
    ...
    def test_regress_candidate_allowlist_expands_per_trace_scenario(self) -> None:
        ...
        def enums(k):
            ...
            # the governor runs ValuePredicate objects; the canonical JSON map
            # is the separate shape the config hash is taken over
            return [v for predicates in vps.values() for p in predicates for v in p.allowed]
