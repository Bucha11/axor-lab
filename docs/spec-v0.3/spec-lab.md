...
> **Superseded as the product narrative** by
> [`docs/spec-suite-platform/`](../spec-suite-platform/) (Experiment Suite
> Platform RFC). That pack governs: Axor Lab is a reproducible experiment
> platform and governance is an *optional capability*, not the spine. This
> document remains accurate as the description of what the code does today and
> as the source of the governance capability's semantics — see
> [`INTEGRATION_PLAN.md`](../spec-suite-platform/INTEGRATION_PLAN.md) for the
> migration. The contract docs in `contracts/` stay authoritative until that
> plan's Phase 1 replaces them.
...
**This spec is the product narrative; the engineering contract lives in `contracts/`** — 10 JSON Schemas (artifact, attestation, bundle, condition, evidence-case, experiment, publication, regression, scenario, suite), plus statistics.md, claims.md, provenance-semantics.md, domain-model.md, the protocol/threat/lifecycle/handoff docs, mvp-contract.md, and a fully-worked vertical-slice.md. This list used to name trace, tool-manifest and predicate as files of their own and omit artifact, suite, evidence-case and regression: a trace belongs to axor-core, and a manifest or a predicate is defined inside the schema that carries it. A no-dependency `validate_slice.py` machine-checks every slice example against the schemas (`cd contracts && PYTHONPATH=.. python3 validate_slice.py`; currently green: 14 examples). Where this narrative and a contract disagree, the contract wins. The vertical slice is the readiness test: when it runs locally on simulated tools and reproduces from its bundle, the core exists.
